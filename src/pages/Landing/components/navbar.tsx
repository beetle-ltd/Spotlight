...
import Logo from "../../../assets/Horizontally stacked black text and yellow-black icon.svg";
      ...
      <div className="w-[90%] mx-auto px-4 flex items-center justify-between">
        ...
        >
          <Link to="/" aria-label="Home">
            <img src={Logo} alt="spotlight logo" className="w-28 md:w-36" />
          </Link>
        </motion.div>
...
const MobileMenu = ({ menuItems, toggleMenu }) => (
  <motion.div
  ...
  >
    <ul className="space-y-4 p-4 w-[90%] mx-auto">
      {menuItems.map((item) => (
        <motion.li
          key={item.id}
          initial={{ opacity: 0, x: -20 }}
        ...
        >
          <Link
            to={item.href}
            className="block text-text hover:text-primary transition-colors duration-300 text-lg py-2"
            onClick={toggleMenu}
          >
            {item.title}
          </Link>
